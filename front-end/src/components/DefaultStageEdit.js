...
function DefaultStageEdit() {
    ...
    useEffect(() => {
        const fetchStage = async () => {
            if (!businessId || !stageId || !adminApiKey) {
        ...
        fetchStage();
    }, [businessId, stageId, adminApiKey]);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            // Navigate back to business page after successful update
            navigate(`/business/${businessId}`);
        } catch (err) {
            console.error('Error updating default stage:', err);
            setError(`Failed to update default stage: ${err.message}`);
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            // Navigate back to business page after successful deletion
            navigate(`/business/${businessId}`);
        } catch (err) {
    ...
    return (
        <Box sx={{ mt: 2 }}>
            ...
            >
                ...
                <DialogContent>
                    <Typography>
                        Are you sure you want to delete the default stage "{stage?.stage_name}"? This action cannot be undone.
                    </Typography>
                </DialogContent>
                <DialogActions>
...
export default DefaultStageEdit;